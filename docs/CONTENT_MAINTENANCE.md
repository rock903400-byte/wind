...
> 作品展開按鈕為「顯示全部作品 ↓ / 收合作品列表 ↑」，文案聚焦於質化與完整呈現，不強調具體卡數。
...
| `hero .scroll-cue` | 比較期導流 | `先看作品實績再決定 →` | `index.html:350` |
...
- `applyView(animate)` 是收折與篩選的單一入口。三種狀態的規則:
  - 選了分類 → 一律全顯示該分類、隱藏「顯示全部」鈕（否則會出現「只剩 2 張卻掛著顯示全部」）
  - 「全部」→ 依 `expanded` 決定收折
...
- **名片正反面內容**：
  - **正面 (Wind)**：`石誠風 Wind` ｜ `全端系統架構師 · 簽約交付` ｜ `協助中小企業把營運雜事系統化` ｜ `跨產業已上線實績 · 橫跨 11+ 產業｜10+ 年金融內控底子` ｜ `100% 原始碼交付`、`提供上線保固與維運`、`合約保障·開立發票` ｜ LINE/電話 `0980463400`、Email `rock90340@gmail.com`、統編 `54730503` ｜ 向量 QR 導向作品集主頁 `index.html`。
