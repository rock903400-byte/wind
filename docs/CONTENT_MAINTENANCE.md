...
- **名片正反面內容**：
  - **正面 (Wind)**：`石顧問 Wind` ｜ `全端接案架構師` ｜ `跨產業全端接案開發 × 系統架構師` ｜ `19 項上線實績 · 橫跨 11+ 產業` ｜ `金融風控系統`、`20萬會員 LINE AI`、`品牌電商 POS` ｜ LINE/電話 `0980463400` (LINE ID: `0980463400`)、Email `rock90340@gmail.com`、統編 `54730503` ｜ 向量 QR 導向作品集主頁 `index.html`。
  - **背面 (飛律 Feilu)**：`飛律 Feilu` ｜ `AI 流程賦能` ｜ `儲值 NT$ 1,000 交付 2 項自動化模組` ｜ `48h 極速交付 · 點數永久 · 100% 自主控管` ｜ 四大模組：`財務自動對帳`、`LINE 智慧秒回`、`規章法規秒查`、`標案情報監控` ｜ LINE/預約 `0980463400`、`純離線/BYOK 私有架構 · 零中繼`、統編 `54730503` ｜ 向量 QR 導向銷售頁 `ai-enablement.html`。
- **雙模式操作方式**：
