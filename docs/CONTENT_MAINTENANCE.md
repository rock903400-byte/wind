...
## 實體商務名片 (`print-card.html`) 送印與維護說明

- **送印與列印工具**：`print-card.html`（獨立 1:1 實體尺寸預覽、另存 PDF、下載高解析 PNG / 向量 SVG）。
- **標準印刷規格**：
  - 成品尺寸：`90mm × 54mm`（台灣標準名片一模尺寸，橫式 5:3）。
  - 送印含出血尺寸：`92mm × 56mm`（四邊各 1mm 出血，上傳印刷廠如健豪、藍格、卡之屋專用）。
  - 安全邊距：文字與重要內容距離裁切邊緣 ≥ 3mm。
- **名片正反面內容**：
  - **正面 (Wind)**：`石顧問 Wind` ｜ `全端接案架構師` ｜ `跨產業全端接案開發 × 系統架構師` ｜ `19 項上線實績 · 橫跨 11+ 產業` ｜ `金融風控系統`、`20萬會員 LINE AI`、`品牌電商 POS` ｜ LINE/電話 `0980463400`、Email `rock90340@gmail.com`、統編 `54730503` ｜ 向量 QR 導向作品集主頁 `index.html`。
  - **背面 (飛律 Feilu)**：`飛律 Feilu` ｜ `AI 流程賦能` ｜ `儲值 NT$ 1,000 交付 2 項自動化模組` ｜ `48h 極速交付 · 點數永久 · 100% 自主控管` ｜ 四大模組：`財務自動對帳`、`LINE 智慧秒回`、`規章法規秒查`、`標案情報監控` ｜ LINE/預約 `0980463400`、`純離線/BYOK 私有架構`、統編 `54730503` ｜ 向量 QR 導向銷售頁 `ai-enablement.html`。
- **送印操作步驟**：
  1. 瀏覽器開啟 `print-card.html`。
  2. 點擊右上角「**含出血 92×56mm**」。
  3. 點擊「**🖨️ 另存 1:1 PDF / 列印**」，列印視窗中將目的地設為「另存為 PDF」，邊界設為「無」，即可取得標準 1:1 正反雙面送印檔。
  4. 亦可點擊「下載正面 (PNG)」與「下載背面 (PNG)」取得 300+ DPI 超高解析圖檔。
